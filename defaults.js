...
//   2) 组合模板：诱饵词 × 引流动作，两个诱饵词挨在一起，或匹配承诺 × 联系动作；
//      单边出现不动，正常发言基本踩不到
// 加规则前先看能不能往下面的词表里补一个词，而不是新加一条规则。
...
/**
 * 强诱饵：日常中文里基本不会出现，配任意一个引流动作即判定。
 */
// eslint-disable-next-line no-unused-vars
const XF_BAIT_STRONG = [
  // 性暗示
...
/**
 * 弱诱饵：引流文案爱用，但日常中文里本义更常见（快餐店、上门维修、
 * 兼职招聘、体制内工作）。只跟强引流动作配对 —— 单配「私信」「主页」
 * 这类每个自我推广账号都写的词，挡下的几乎全是正常人。
 */
// eslint-disable-next-line no-unused-vars
const XF_BAIT_WEAK = [
  "顶\\s*不[\\s\\S]{0,2}住",
...
/** 诱饵词全表：两个诱饵词挨在一起（d-bait-bait）按这一张判定。 */
// eslint-disable-next-line no-unused-vars
const XF_BAIT = [XF_BAIT_STRONG, XF_BAIT_WEAK].join("|");

/**
 * 强联系动作：站外联系方式与拉客话术，正常账号写不到。
 */
const XF_CONTACT_STRONG = [
  "不\\s*信\\s*你\\s*看|你\\s*看\\s*(就|便)\\s*知",
  "加\\s*我",
  "约\\s*不\\s*约|约\\s*起|处\\s*一\\s*个|处\\s*个",
  "磁\\s*力|种\\s*子|onlyfans",
  "秒\\s*删",
  "同\\s*号|付\\s*费\\s*群",
  "滴\\s*我",
  "扣\\s*(我|1|一)",
  ...
  // 归一化会删掉点和斜杠，t.me/xhjj 到匹配这一步是 tmexhjj；
  // 前面不许接字母，免得咬到 apartment / treatment 里的同形片段
  "(?<![a-z])tme",
  "无\\s*套\\s*路",
  "打\\s*[✈飞]",
  // 「附近👇来🈷我」：🈷 归一化后是「月」
...
/**
 * 弱联系动作：把人导向主页或私聊，但正经账号天天这么写
 * （合作私信、简介有邮箱、主页有长文）。只跟强诱饵配对。
 */
const XF_CONTACT_WEAK = [
  "主\\s*页",
  "简\\s*介",
  "置\\s*顶",
  "头\\s*像",
...
/** 联系动作全表：d-match-cta 用它跟匹配承诺配对。 */
const XF_CONTACT = [XF_CONTACT_STRONG, XF_CONTACT_WEAK].join("|");

/**
 * 服务承诺：撮合与覆盖范围的话术，本身不含联系方式。
 * 单独拆出来，是因为「实时匹配」这类写法自己就落在这一层，
 * 拿它跟联系动作配对才有意义（见 d-match-cta）。
 */
const XF_PROMISE_STRONG = [
  "速\\s*配",
  "覆\\s*盖\\s*全\\s*国",
...
/** 弱服务承诺：同城交友、活动匹配都是正常说法。 */
const XF_PROMISE_WEAK = ["同\\s*城", "匹\\s*配"].join("|");

/** 引流动作全表。 */
// eslint-disable-next-line no-unused-vars
const XF_CTA = [XF_CONTACT, XF_PROMISE_STRONG, XF_PROMISE_WEAK].join("|");

/** 强引流动作：弱诱饵词只认这一层。 */
const XF_CTA_STRONG = [XF_CONTACT_STRONG, XF_PROMISE_STRONG].join("|");
...
const XF_DEFAULT_RULES = [
  ...
  },
  {
    ...
    pattern:
      ...
      // 「给你看福」：展示动作 + 福，后面排掉福建 / 福利 / 福气等正常词
      "|[给带让]\\s*[你我他她]?\\s*看\\s*福(?!\\s*(建|州|利|气|音|报|星|袋|田|尔))" +
  ...
  // --- 2. 组合模板：变体再多也不用一条条加 ---
  {
    ...
    //
    // 两边都按强弱分层，弱 × 弱 不判定。匹配用的文本里含作者简介，
    // 而简介对这个作者的每一条推文都一样 —— 门槛低一档，挡下的就不是一条
    // 推文而是整个账号：「兼职KOL，合作私信」这种正经简介正好凑齐一组弱信号。
    pattern:
      `^(?:(?=[\\s\\S]*(?:${XF_BAIT_STRONG}))(?=[\\s\\S]*(?:${XF_CTA}))` +
      `|(?=[\\s\\S]*(?:${XF_BAIT_WEAK}))(?=[\\s\\S]*(?:${XF_CTA_STRONG})))`,
    type: "regex",
    enabled: true,
  },
  {
    id: "d-match-cta",
    label: "匹配承诺 × 引流动作",
    // 「实时匹配 · 覆盖全国 · 主页联系」这类昵称整段都是引流动作，一个诱饵词
    // 都不带，前两条组合层接不住。匹配承诺要带修饰词才算，「活动匹配到同城
    // 用户」这种正常说法踩不到。
    pattern:
      "^(?=[\\s\\S]*(?:(?:实\\s*时|智\\s*能|精\\s*准|在\\s*线|真\\s*人|同\\s*城|附\\s*近|全\\s*国|随\\s*机)" +
      "\\s*(?:匹\\s*配|速\\s*配|配\\s*对)|秒\\s*配|(?:匹\\s*配|配\\s*对)\\s*(?:覆\\s*盖|全\\s*国)))" +
      `(?=[\\s\\S]*(?:${XF_CONTACT}))`,
    type: "regex",
    enabled: true,
  },
